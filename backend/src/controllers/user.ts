...
import { SPLAGENRequest } from "../middleware/auth";
import { firebaseAuth } from "../services/firebase";
import UserModel, { DisplayUser, UserRole } from "../models/user";
 ...
 */
export const getWhoAmI: RequestHandler = async (req: SPLAGENRequest, res, next) => {
  try {
    const { userUid } = req;
 ...
 */
export const getUsers: RequestHandler = async (req: SPLAGENRequest, res, next) => {
  try {
    const SPLAGENUsers = await UserModel.find();
    const displayUsers: DisplayUser[] = [];
    for (const SPLAGENUser of SPLAGENUsers) {
      const { uid, _id } = SPLAGENUser;
 ...
 */
export const createUser: RequestHandler = async (req: SPLAGENRequest, res, next) => {
