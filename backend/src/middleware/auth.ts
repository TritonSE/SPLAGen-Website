 ...
 */
const requireSignedIn = async (req: SPLAGENRequest, res: Response, next: NextFunction) => {
 ...
 */
const requireAdmin = async (req: SPLAGENRequest, res: Response, next: NextFunction) => {
  const { userUid } = req;
  const user = await UserModel.findOne({ uid: userUid });
